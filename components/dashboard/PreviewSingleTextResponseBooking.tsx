...
const PreviewSingleTextResponseBooking = ({
    ...
    const [bookingOpened, setBookingOpened] = React.useState(
        queryParams.booking === booking._id
    );

    useEffect(() => {
        // If the queryParams changes, and if the booking id in the queryParams
        // matches the booking id in the component, then set the bookingOpened
        // state to true. Else, set it to false.
        if (queryParams.booking === booking._id) {
            ...
            <div className="flex flex-col justify-between gap-3 w-full h-fit py-5 bg-white border-b min-w-[20rem]">
                ...
                <div className="flex justify-between items-center gap-2">
                    ...
                    {/* Button that when clicked, pushes the specific booking associated
                        ...
                        If the booking is opened, doesn't show.
                        */}
                    <div className="flex gap-1">
                        {!bookingOpened ? (
                            ...
                            <div className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md bg-transparent">
                                Opened
                            </div>
                        )}
                    </div>
